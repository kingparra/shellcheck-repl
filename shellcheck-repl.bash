...
sc_repl_verify_or_unbind() {
	...
	local skip_pattern=${SHELLCHECK_REPL_SKIP_PATTERN:-(^\!|[[:space:]][[:space:]]$)}
	...
	case ${style} in
		...
		*) >&2 echo "ERROR: Unknown value for shellcheck-repl " \
			"variable 'SHELLCHECK_REPL_INFO' (valid values" \
			"are 'raw', 'full', 'short' and 'clean'" \
			"[default]): '${SHELLCHECK_REPL_INFO}'"
		;;
	esac
